...
class ChartEngine: ObservableObject {
    ...
    func zoomIn() {
        let newZoom = min(zoomLevel * 1.2, maxZoom)
        setZoom(newZoom)
    }
    
    func zoomOut() {
        let newZoom = max(zoomLevel / 1.2, minZoom)
        setZoom(newZoom)
    }
    ...
    private func calculateVisibleRange() {
        ...
        let panAdjustment = Int(panOffset / 10) // Adjust sensitivity
        let startIndex = max(0, min(totalCandles - candlesToShow, totalCandles - candlesToShow - panAdjustment))
...
extension ChartEngine {
    func candlestickPath(for dataPoint: OHLCDataPoint, at index: Int, in geometry: GeometryProxy) -> Path {
        ...
        let candleWidth = geometry.size.width / CGFloat(visibleRange.count) * 0.8
    ...
    func areaPath(in geometry: GeometryProxy) -> Path {
        ...
        if let firstDataPoint = visibleData.first {
            ...
            path.addLine(to: CGPoint(x: firstX, y: yPosition(for: firstDataPoint.close, in: geometry)))
        }
